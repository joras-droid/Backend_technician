...
function getChartLabels(duration) {
    switch (duration) {
        case 'daily':
...
let ReportsService = class ReportsService {
    ...
    async getTimeSummary(query, callerRole) {
        ...
        const technicianList = Object.values(technicians).map((tech) => ({
            ...tech,
            totalHours: parseFloat(tech.totalHours.toFixed(2)),
            averageHoursPerWorkOrder: parseFloat((tech.totalHours / tech.workOrdersCount).toFixed(2)),
        ...
        return {
            summary: {
                ...
                averageHoursPerWorkOrder: parseFloat((totalHours / workOrdersCount || 0).toFixed(2)),
            },
    ...
    async getDashboardMetrics(duration = 'weekly', callerRole) {
        ...
        const inProgressCount = workOrders.filter((wo) => wo.status === 'ACTIVE' &&
            wo.timeEntries.some((te) => te.checkInAt && !te.checkOutAt)).length;
        ...
        const changePct = prevWorkOrders > 0
    ...
    async getRecentActivities(limit = 20, callerRole) {
        ...
        workOrders.forEach((wo) => {
            ...
            const statusLabel = wo.status === 'COMPLETED' ? 'completed' : wo.status === 'PAID' ? 'closed' : 'pending';
            activities.push({
                id: `wo_${wo.id}`,
                title: isNew
