...
export class EmailService {
  ...
  async sendPasswordResetOtp(to: string, otp: string, firstName?: string): Promise<void> {
    const fromName = this.configService.get<string>('EMAIL_FROM_NAME', 'Technician Management System');
    const from = this.configService.get<string>('EMAIL_FROM', 'noreply@technician.com');
  ...
  private getPasswordResetOtpTemplate(otp: string, displayName: string): string {
    return `
<!DOCTYPE html>
<html>
<head>
